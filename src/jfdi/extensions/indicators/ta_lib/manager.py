...
from jfdi.extensions.indicators.ta_lib.common import (
    output_suffix_map,
    taf_params_re,
...
class TAFunctionWrapper:
    ...
    def __eq__(self, other):
        return (
            isinstance(other, TAFunctionWrapper)
    ...
    def _get_outputs_names(name: str, fn: abstract.Function) -> list[str]:
        ...
        output_names = [
            (
                name + suffix + output_suffix_map.get(output_name, f"_{output_name}")
            ).upper()
...
class TALibIndicatorManager(Indicator):
    ...
    def handle_bar(self, bar: Bar) -> None:
        ...
        PyCondition.equal(
            bar.bar_type, self._bar_type, "bar.bar_type", "self._bar_type"
        )
